...
<div class="container">
    ...
    <form action="{{ route('penyimpanan.update', $penyimpanan->id) }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <select name="id_opname" id="id_opname" class="form-control" onchange="updatePengadaan()">
                @foreach ($opname as $item)
                    <option value="{{ $item->id_opname }}" {{ $penyimpanan->id_opname == $item->id_opname ? 'selected' : '' }}>
                        {{ $item->pengadaan->kode_pengadaan }}
                    </option>
                @endforeach
            </select>
            @error('id_opname')
        ...
        <div class="mb-3">
            ...
            <select name="id_lokasi" id="id_lokasi" class="form-control">
                @foreach ($lokasi as $item)
                    <option value="{{ $item->id_lokasi }}" {{ $penyimpanan->id_lokasi == $item->id_lokasi ? 'selected' : '' }}>
                        {{ $item->nama_lokasi }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="nama_barang" id="nama_barang" class="form-control" value="{{ $penyimpanan->nama_barang }}">
            @error('nama_barang')
                <div class="form-text text-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <input type="number" name="jumlah" id="jumlah" class="form-control" value="{{ $penyimpanan->jumlah }}" oninput="validateJumlah()">
            @error('jumlah')
                <div class="form-text text-danger">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-3">
            ...
            <select name="status" id="status" class="form-control">
                <option value="baik" {{ $penyimpanan->status == 'baik' ? 'selected' : '' }}>Baik</option>
                <option value="rusak" {{ $penyimpanan->status == 'rusak' ? 'selected' : '' }}>Rusak</option>
                <option value="hilang" {{ $penyimpanan->status == 'hilang' ? 'selected' : '' }}>Hilang</option>
            </select>
            @error('status')
                <div class="form-text text-danger">{{ $message }}</div>
            @enderror
